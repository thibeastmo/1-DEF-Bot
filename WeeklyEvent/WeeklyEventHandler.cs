    ...
    public class WeeklyEventHandler
    {
        ...
        public async Task<List<WeeklyEventType>> CheckAndHandleWeeklyEvent(WGBattle battle)
        {
            ...
            if (battle.vehicle == WeeklyEvent.Tank)
            {
                ...
                await UpdateLastWeeklyEvent(this.WeeklyEvent.GenerateEmbed());
            }
        ...
        public async Task ReadWeeklyEvent()
        {
            try
            {
                ...
                if (weeklyEventChannel != null && weeklyEventChannel.GetMessagesAsync(1).Result.Count > 0)
                {
                    DiscordMessage message = await weeklyEventChannel.GetMessageAsync(weeklyEventChannel.LastMessageId.Value);
                    if (message != null)
                    {
        ...
        public async Task UpdateLastWeeklyEvent(DiscordEmbed embed)
        {
            await this.DiscordMessage.ModifyAsync(embed);
        }
